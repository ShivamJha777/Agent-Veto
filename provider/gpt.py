...
def chat(prompt: str, model: str = "chatgpt-4o-latest",stream=True) -> str:
    """
    Generate response from ChatGPT.es API.
    ...
    Returns:
        str: Response generated.
    """
    ai = ChatGPTES(model=model)
    print("AI: ", end="")
    res = ai.chat(prompt, stream=stream)
    response = ""
    if stream:
        for chunk in res:
            print(chunk, end="")
